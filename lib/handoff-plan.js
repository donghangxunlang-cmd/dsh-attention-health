...
export function deriveCompactionPlan(input = {}) {
  ...
  const measuredBaseline =
    hasMeasured && typeof last.afterUsedTokens === 'number' && last.afterUsedTokens > 0
      ...
      : null;
  const estimatedBaseline = Math.round(usedTokens * (1 - T.compressibleRatio));
  ...
  // （`headroomRounds`）—— 过了官方线，官方会先做零 token 的无损 prune，游戏规则就变了。
  //
  ...
  const compactReason = {
    ...
    recommend: `占声明窗口 ${fmt(windowPercent)}（${T.compactRecommendFrom}~${T.compactUrgentFrom}%）—— 压不压都不影响继续；到官方线官方会先做零 token 的无损裁剪。`,
    urgent: nearOfficial
      ? `占声明窗口 ${fmt(windowPercent)}，已接近官方自动压缩线（${T.officialCompactPercent}%）—— 到线官方会先做零 token 的无损裁剪，不会直接摘要有损内容。`
  ...
  // 官方 `dsh-compaction-basic` 到线时**先做零 token 的无损裁剪（prune）**，只有裁剪不够
  // 才调模型生成摘要；而手动 `/compact` 是**立刻**调模型、立刻付全价、立刻承担有损。
  ...
  if (qualityAdvice === 'immediate') {
    ...
    //   1) 只数**模型摘要**。`compaction/prune` 是零 token、无损的裁剪，压几次都不丢信息，
    //      拿它去劝人交接并不成立。官方 `dsh-compaction-basic` 在 80% 线触发时是
    ...
    finalReason =
      ...
      '到线时官方会**先做零 token 的无损修剪**（裁剪超大工具结果），修剪后低于阈值就完全跳过摘要；' +
    ...
    if (dataMissing) {
      ...
      finalReason =
        ...
        `到官方自动压缩线（${T.officialCompactPercent}%）时官方会自己处理（**先做零 token 的无损裁剪**）。`;
    } else {
      finalReason =
  ...
  // ── 「准备交接」中间档（AF.2 需求 2，2026-09-20 用户提出）────────────────────
  //
  // 为什么加：原先只有 continue / handoff 两档，"**差一点**"的情况被归进"可以继续"，
  // 用户既看不出边界快到了，也没有"在任务边界顺手交接"的余地。
  //
  // 语义（用户原话）：不催促、不否定继续，给"心理准备 + 边界选择权"。
  // 两个触发条件（**都要求其它成本门槛先过** —— 否则那是"算不出来"，不是"接近"）：
  //   ① 回本已落在窗口内、但余量不足：`0 ≤ 余量 ≤ prepareMarginRounds`
  //      （handoff 要求余量 > `handoffMarginRounds`，所以这条带与 handoff 不重叠）；
  //   ② 节省额接近但未达门槛：`prepareSavingRatio × minSavingYuan ≤ savingYuan < minSavingYuan`。
  //
  // ⚠️ 只在**成本通道**成立，且**只把原本的 `continue` 升级**（下面接在整条判定链之后）——
  // 不去覆盖质量 / 官方线 / 推迟压缩这些已经想清楚的结论。
  {
    const costGatesOpen =
      priceUsable &&
      hitRateKnown &&
    ...
    const nearSavingGate =
      savingYuan > 0 && savingYuan < minSaving && savingYuan >= minSaving * (T.prepareSavingRatio ?? 0.6);
    const thinMargin =
      freshBreakEvenRounds !== null &&
    ...
    if (
      ...
      // 结论是"到线官方会自己先做无损裁剪，无需手动操作"）与质量强制覆盖（`override`）
