...
// AG 节（2026-09-22）：`ADVICE_VALUES` 是 `final.advice` 取值的**单一来源** ——
// `index.js` 的 `viewSchema` 直接用它构造 `z.enum`，不再手写第二份清单
//（漏一档的后果是**整条 view 校验失败**，而 `costNote` 在真实语料上 0 触发，测试看不出来）。
export { HIT_RATE_WINDOW, FINAL_NOTES, ADVICE_VALUES, deriveCompactionPlan, formatBreakEven } from './handoff-plan.js';
