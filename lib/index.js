...
const viewSchema = z
  .object({
    ...
    /**
     * 最终建议（质量优先于压缩）：继续 / 留意成本 / 准备交接 / 机械交接。
     *
     * ⚠️ 取值清单来自 `ADVICE_VALUES`（`handoff-plan.js`）—— **不要在这里手写第二份**。
     * AG（2026-09-22）新增 `costNote` 时就是手写枚举漏了一档：后果不是"那一档降级"，
     * 而是**整条 view 校验失败**；而它在真实语料上触发 0 次
     * （`work/ag-gradient-scan.mjs` 实测 82 个会话），测试与线上都看不出来，
     * 只有真出现一个"省额很小"的会话才会炸。单一来源 + 源码级守卫（`plugin-test`）钉住它。
     */
    compactAdvice: z.enum(ADVICE_VALUES),
    /** 最终建议文案（一句话）。 */
...
function finalize(base) {
  ...
  return {
    ...
    //
    // AG（2026-09-22）：新增的最轻档 `costNote` **照旧点亮**（它是浅蓝、中点、最轻字重，
    // 文案只说"每轮成本偏高，继续没问题"，不含任何命令性措辞）。
    // 为什么不让它静默：W 节静默针对的是"**仅规模偏大**"这种无行动价值的状态，
    // 而 `costNote` 陈述的是**钱**（用户明确关心的那一件），且它只在
    // "省额为正、但不到门槛 60%" 的窄带里出现 —— 实测 82 个真实会话上触发 **0 次**
    // （`work/ag-gradient-scan.mjs`），不会稀释真正的信号。
