...
export function renderHandoff(x, meta = {}) {
  ...
  // S6（AT 节 2026-09-24）：H 是**对比窗口**，不是「只能再跑 H 轮」——
  // 不写明，读者会把「到官方线前总花费」读成"这段工作只能持续 H 轮"。
  push(
    `> 表中的「到官方线前总花费」以 **H = ${plan.cost.horizonRounds} 轮**` +
      '（距 80% 自动压缩线的预计轮数）为**对比窗口**；H 只是比较口径，' +
      '**不代表**压缩 / 交接之后只能再跑 H 轮。',
  );
  // S5（AT 节 2026-09-24）：压缩成本**极不确定**（摘要请求能否复用缓存，实测相差 28 倍）——
  // 只给一个点值，读者会把它当成账单。
  push(
    '> **压缩成本取决于摘要请求能否复用缓存**：实测区间 ¥0.017 ~ ¥0.483（约 **28 倍**），' +
      '中位数约 ¥0.023 —— 上表 B 行是估算，不是账单。',
  );
