    ...
    function AttentionHealthBar(props) {
      ...
      const head = React.createElement(
        ...
        React.createElement(
          ...
          // AF.2（2026-09-20）：`prepare` 与 `handoff` **都是黄色**，靠图形与字重区分 ——
          //   prepare：空心点「○」普通字重（留意，不催促）
          //   handoff：实心点「●」加粗（明确建议）
          // 有质量信号时统一是橙色「⚠」（退化 / 守卫），与"建议"语义分开。
          hasSignal ? '⚠' : health.compactAdvice === 'prepare' ? '○' : '●',
        ),
        React.createElement(
          'span',
          { style: { color: adviceColor, fontWeight: health.compactAdvice === 'handoff' ? 600 : 500 } },
          hasSignal
      ...
      if (typeof health.costContinuePerRound === 'number') {
        ...
        if (showCostDetail && health.costPriceKnown === false) {
          ...
          const bits = ['继续 ' + yuan(health.costContinuePerRound)];
          if (typeof health.costFreshPerRound === 'number') {
